...
# Auth
from routes.auth import router as auth_router  # ← YA NO importamos get_current_user
...
async def crear_producto(payload: ProductoIn, user: dict = Depends(get_current_user)):
    data = await read_db()
...
async def actualizar_producto(producto_id: int, payload: ProductoIn, user: dict = Depends(get_current_user)):
    data = await read_db()
...
async def eliminar_producto(producto_id: int, user: dict = Depends(get_current_user)):
    data = await read_db()
    productos = data.get("productos", [])
